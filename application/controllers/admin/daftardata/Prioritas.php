...
class Prioritas extends OperatorBase
{
    ...
    public function index()
    {
        ...
        $params = $data_session['session_operator']['instansi_cd'];
        
        // SET TEMPLATE CONTENT
        $this->tsmarty->assign("template_content", "admin/daftardata/prioritas/index.html");
        ...
        // GET DATA INDICATOR PRIORITAS STATISTIK 
        if (!empty($params)) {
            $rs_id = $this->M_indicator_pg->get_indicator_by_prioritas_statistik_by_opd($params);
            $geo = $this->M_indicator_pg->get_indicator_by_prioritas_geospasial_by_opd($params);
        } else {
            $rs_id = $this->M_indicator_pg->get_indicator_by_prioritas_statistik();
            $geo = $this->M_indicator_pg->get_indicator_by_prioritas_geospasial();
        }

        // ASSIGN DATA
        $this->tsmarty->assign("rs_id", $rs_id);
        $this->tsmarty->assign("geo", $geo);
    ...
    public function download_data_instansi($instansi_cd = "")
    {
        ...
        if (empty($instansi)) redirect("admin/daftardata/prioritas/indicator");
    ...
    public function urusan($instansi_cd)
    {
        ...
        $this->_set_page_rule("R"); //enable
        // SET TEMPLATE CONTENT
        $this->tsmarty->assign("template_content", "admin/daftardata/prioritas/urusan.html");
    ...
    public function indicator($instansi_cd = "", $data_id = "")
    {
        ...
        $this->_set_page_rule("R"); //enable
        // SET TEMPLATE CONTENT
        $this->tsmarty->assign("template_content", "admin/daftardata/prioritas/indicator.html");
    ...
    public function search_process()
    {
        ...
        $url = "admin/daftardata/prioritas/indicator/" . $instansi_cd . '/' . $urusan_id;
        redirect($url);
    }
    ...
    public function ajukan_process()
    {
        ...
        // REDIRECT
        redirect("admin/daftardata/prioritas/indicator/" . $instansi_cd . "/" . $urusan_id);
    }
    ...
    public function search_urusan_process()
    {
        ...
        if (empty(trim(strip_tags($this->input->post('instansi_cd', TRUE)))))
            ...
            redirect("admin/daftardata/prioritas/urusan/" . trim(strip_tags($this->input->post('instansi_cd', TRUE))));
    }
}
