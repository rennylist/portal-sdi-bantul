...
class Statistik extends OperatorBase
{
    ...
    public function urusan($instansi_cd)
    {
        ...
        if (empty($instansi)) redirect("admin/monitoring/statistik");
    ...
    public function indicator($instansi_cd = "", $data_id = "")
    {
        ...
        if (empty($urusan)) redirect("admin/monitoring/statistik");
    ...
    public function search_urusan_process()
    {
        ...
        if (empty(trim(strip_tags($this->input->post('instansi_cd', TRUE)))))
            redirect("admin/monitoring/statistik");
        else
    ...
    public function upload()
    {
        ...
        $this->_set_page_rule("R");
        // SET TEMPLATE CONTENT
        $this->tsmarty->assign("template_content", "admin/monitoring/statistik/upload.html");
    ...
    public function upload_process()
    {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (empty($_FILES['template']['tmp_name'])) {
                ...
                // REDIRECT
                redirect("admin/monitoring/statistik/upload");
            } else {
                // SET FILE NAME
                $tahun =  trim(strip_tags($this->input->post('tahun', TRUE)));
                ...
                if ($this->tupload->do_upload('template')) {
                    ...
                    // REDIRECT
                    redirect("admin/monitoring/statistik/index");
                }
            }
        }
    }
    ...
    public function upload_verifikasi()
    {
        ...
        $this->_set_page_rule("R");
        // SET TEMPLATE CONTENT
        $this->tsmarty->assign("template_content", "admin/monitoring/statistik/upload_verifikasi.html");
    ...
    public function upload_process_verifikasi()
    {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            if (empty($_FILES['template']['tmp_name'])) {
                ...
                // REDIRECT
                redirect("admin/monitoring/statistik/upload_verifikasi");
            } else {
                // SET FILE NAME
                $tahun =  trim(strip_tags($this->input->post('tahun', TRUE)));
                // $instansi_cd =  trim(strip_tags($this->input->post('instansi_cd', TRUE)));
