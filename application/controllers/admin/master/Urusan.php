...
class Urusan extends OperatorBase
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/master/M_urusan');
    ...
    public function index()
    {
        ...
        $this->tsmarty->assign("template_content", "admin/master/urusan/index.html");
        ...
        /* start of pagination --------------------- */
        // pagination
        $config['base_url'] = site_url("admin/master/urusan /index/");
        $config['total_rows'] = $this->M_urusan->get_total_urusan(array($user_st, $user_alias, $user_alias));
    ...
    public function edit_account_process()
    {
        ...
        $this->tnotification->set_rules('send_email', 'Kirim Email', 'trim|required');
        // check email
        $email = trim($this->stripHTMLtags($this->input->post('user_mail', TRUE)));
        $email_lama = trim($this->stripHTMLtags($this->input->post('user_mail_old', TRUE)));
        if ($email != $email_lama) {
        ...
        if ($this->tnotification->run() !== FALSE) {
            ...
            $user_pass = $this->encryption->encode(md5($password), $password_key);
            ...
            if ($this->M_user->update_user($params, $where)) {
                // send email
                $send_mail = $this->stripHTMLtags($this->input->post('send_email', TRUE));
                if ($send_mail === '1') {
                    // config
