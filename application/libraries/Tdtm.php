...
class tdtm {
    ...
    function __construct() {
        ...
        $this->arr_lang['ins'] = array('00' => 'ERR', '' => 'ERR', '0' => 'ERR', '01' => 'Jan', '02' => 'Feb', '03' => 'Mar', '04' => 'Apr', '05' => 'Mei', '06' => 'Jun', '07' => 'Jul', '08' => 'Ags', '09' => 'Sep', '10' => 'Okt', '11' => 'Nov', '12' => 'Des');
        // english short date
    ...
    public function get_bulan_singkatan($month) {
        if($month == ''){
            return "ERR";
        }else{
            return $this->arr_lang['ins'][$month];
        }
        // return $month;
    }
    // get bulan romawi
    ...
    public function get_full_date_local($tgl, $lang = "in") {
        // check input tanggal
        $tgl = explode(' ', $tgl);
        $tgl_day = isset($tgl[0]) ? $tgl[0] : '00-00-0000';
        $tgl_jam = isset($tgl[1]) ? $tgl[1] : '';
        ...
        // parse date
        $month_label = isset($this->arr_lang[$lang][$tgl_day[1]]) ? $this->arr_lang[$lang][$tgl_day[1]] : $this->arr_lang[$lang]['00'];
        $date_label = $tgl_day[0] . ' ' . $month_label . ' ' . $tgl_day[2];
        // parse time
        if (!empty($tgl_jam)) {
            $tgl_jam = $tgl_jam[0] . ':' . $tgl_jam[1] . ':' . $tgl_jam[2];
            $date_label .= ' Jam ' . $tgl_jam;
