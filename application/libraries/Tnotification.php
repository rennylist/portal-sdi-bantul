...
class tnotification extends CI_Form_validation {
    ...
    public function display_notification($open = "<ul>", $close = "</ul>") {
        ...
        $this->get_session();
        // display header
        $this->CI->tsmarty->assign("notification_header", "");
        if (!empty($this->data)) {
            $this->CI->tsmarty->assign("notification_header", $this->data['status']);
            $this->CI->tsmarty->assign("notification_message", $this->data['message']);
            if (!empty($this->data['error'])) {
                $this->CI->tsmarty->assign("notification_error", $open . $this->data['error'] . $close);
            } else {
                $this->CI->tsmarty->assign("notification_error", "");
            }
        }
        // clear session
        $this->clear_session();
    ...
    public function display_last_field($var_name = "result") {
        ...
        if (!empty($this->data['data'])) {
            ...
            // return
            $this->CI->tsmarty->assign($var_name, $output);
        }
